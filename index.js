...
import { getTableNameFromArgs, getKeyFieldsFromArgs, getTypeFromArgs } from "./libs/process/args.js";
import { askQuestion } from "./libs/process/askQuestion.js";

// Get table name from arguments.
const tableName = getTableNameFromArgs();
if (tableName == null) {
    console.log("No tableName provided! Use --help for more info.", tableName);
    process.exit(1);
}
...
const operationType = getTypeFromArgs();
...
const messageString = operationType === "ADD" ? `\nEvery record will have 2 new fields:\n\t1. _version\n\t2. _lastChangedAt` : `\nThe following fields will be removed for each record:\n\t1. _version\n\t2. _lastChangedAt`
...
// Migration function.
const newRecords = migrateData(tableItems, operationType);
...
// Table update.
const results = await updateRecords(tableName, tableKeyFields, newRecords, operationType);
